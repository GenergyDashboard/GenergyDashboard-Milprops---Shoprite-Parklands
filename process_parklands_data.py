"""
process_parklands_data.py

Reads today's scraped data (data/parklands_raw.xlsx) and adds it to the
starting values (data/starting_values.json) to produce an updated
output (data/parklands_processed.json).
...
Flow:
  ...
  5. Save updated parklands_processed.json
...
def main():
    data_dir = Path("data")
    raw_file = data_dir / "parklands_raw.xlsx"
    starting_file = data_dir / "starting_values.json"
    output_file = data_dir / "parklands_processed.json"
    
    print("🔄 Processing Shoprite Parklands data...")
